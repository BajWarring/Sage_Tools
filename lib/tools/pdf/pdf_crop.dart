...
import 'package:path_provider/path_provider.dart'; // Added for safe paths
...
class _PdfCropScreenState extends State<PdfCropScreen> {
  ...
  Future<void> _loadPdfSequence() async {
    try {
      // FIX: Use async read to prevent UI freeze
      final bytes = await File(widget.filePath).readAsBytes();
      
      // 1. Get Vector Info (Rotation/Size)
      final vDoc = vector.PdfDocument(inputBytes: bytes);
      final vPage = vDoc.pages[0];

      // Robust rotation detection
      if (vPage.rotation == vector.PdfPageRotateAngle.rotateAngle90) {
        _pageRotation = 90;
      } else if (vPage.rotation == vector.PdfPageRotateAngle.rotateAngle180) {
        _pageRotation = 180;
      } else if (vPage.rotation == vector.PdfPageRotateAngle.rotateAngle270) {
        _pageRotation = 270;
      } else {
        _pageRotation = 0;
      }
    ...
    setState(() {
      ...
      if (ratio == null) {
        ...
        if (_cropRect.top + newH > _imageSize!.height) {
          ...
          _cropRect = Rect.fromLTWH(
              _cropRect.left, _cropRect.top, currentW, newH);
        }
      }
      _updateControllers();
    });
  ...
  // --- UPDATED MATH: Visual Rect -> Physical Rect ---
  // Calculates the physical rectangle on the unrotated PDF page
  Rect _calculatePhysicalRect(Rect visualRect) {
    ...
    // Map visual coordinates to physical coordinates based on rotation
  ...
  Future<void> _savePdf() async {
    ...
    try {
      // 1. Permission (Android 10+ often doesn't need this for Downloads, 
      // but keeping basic storage req for older devices)
      ...
      // UPDATED LOGIC: Set size to crop dimensions, then apply original rotation
      ...
      // 5. Determine Save Path (Fix for crashes)
      Directory? directory;
      if (Platform.isAndroid) {
